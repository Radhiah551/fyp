...
def get_students():
    try:
        conn = sqlite3.connect("students.db")
        cursor = conn.cursor()
        cursor.execute("SELECT student_id, name, semester, attendance, fees_total, fees_balance FROM students ORDER BY name")
...
# ================= AI SUPPORT (FIXED ONLY HERE) =================
@app.route('/ai-support', methods=['POST'])
def ai_support():
    try:
        data = request.json
        message = data.get("message")
        student_id = data.get("student_id", "123")
...
Name: {student[3]}
Semester: {student[4]}
Current Subjects: {student[5]}
Total Fees: RM {student[6]}
Outstanding Fees Balance: RM {student[7]}
CGPA: {student[8]}
Attendance: {student[9]}%
Quiz Marks: {student[10]}
Test Marks: {student[11]}
Assignment Marks: {student[12]}
Coursework: {student[13]}
Credit Hours Completed: {student[14]} (120 needed to graduate)
Next Semester Subjects: {student[15]}
Pass Status: {student[16]}
"""
        else:
        ...
        try:
            response = client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are a university AI assistant. Reply briefly and clearly using the student's records below. If fees balance > 0, remind student politely.\n\nStudent:\n" + student_info
                    },
            ...
            if not reply:
                reply = "AI did not return a response. Please try again."
...
def add_post():
    try:
        ...
        # Fall back to a random peer name only if no name was supplied
        if not student_name:
        ...
        # AI comment (SAFE FIXED)
        try:
            response = client.chat.completions.create(
                model="llama-3.1-8b-instant",
                messages=[
                    {"role": "system", "content": "You are a friendly university student. Reply briefly."},
                    {"role": "user", "content": message}
                ]
            )
        ...
        # Pick a random peer name for the AI-generated comment
        peer_names = ["Alex","Sarah","Daniel","Aisyah","Jason","Farah","Amir","Sophia","John","Haziq","Emily","Ryan"]
        peer_name = random.choice(peer_names)
        comments = f"<p>💬 <strong>{peer_name}:</strong> {ai_reply}</p>"
        likes = random.randint(1, 5)

        cursor.execute(
            "UPDATE peer_posts SET comments=?, likes=? WHERE id=?",
            (comments, likes, post_id)
        )
...
def comment_post(post_id):
    try:
        ...
        commenter_name = data.get("commenter_name", "Student").strip() or "Student"
        ...
        updated_comments = current + f"<p>💬 <strong>{commenter_name}:</strong> {comment}</p>"

        cursor.execute(
            "UPDATE peer_posts SET comments=? WHERE id=?",
